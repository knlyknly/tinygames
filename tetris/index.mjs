import { TetrisTimerActionProvider } from "./tetris-input.mjs";
...
const restart = (() => {
  const inputActionProvider = new TetrisKeyboardInput(KeyMaps.Keyboard1);
  const timerActionProvider = new TetrisTimerActionProvider();
  let tetris;
  return () => {
    const height = document.querySelector('#rowsInput').value;
    const width = document.querySelector('#colsInput').value;
    if (tetris) {
